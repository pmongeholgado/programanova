...
/* ---------- API LOCAL / RED (DEFINITIVO) ---------- */
/*
  - LOCAL (Go Live/5500): usa backend local 8000
  - RED: puedes definir window.__API_BASE__ en index.html (recomendado)
         ejemplo: window.__API_BASE__ = "https://TU_BACKEND.com";
*/
...
function getApiUrl() {
  ...
  // 1) Si estamos en local, SIEMPRE backend local
  if (isLocal) {
    return "http://127.0.0.1:8000/novap/chat";
  }

  // 2) En red: si definiste base URL del backend, úsala
  const base = (window.__API_BASE__ || "").trim().replace(/\/+$/, "");
  if (base) {
    return `${base}/novap/chat`;
  }

  // 3) En red: si backend y frontend están en el MISMO dominio (reverse proxy),
...
function renderChatList() {
  ...
  chats.forEach(chat => {
    ...
    if (chat.id === activeChatId) container.classList.add("active");
...
function createNewChat() {
  ...
  chats.unshift({
    ...
    messages: [
      { text: "Hola Pablo 👋 Bienvenido a chatNOVAP", sender: "bot" }
    ]
  });
...
async function sendMessage() {
  ...
  const thinkingIndex = chat.messages.push({
